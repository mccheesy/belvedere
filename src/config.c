        ...
        if (*trimmed == '[') {
            ...
            *eq = '\0';
            ...
            if (strcasecmp(key, "setleds") == 0) {
                ...
                while (token && config->monitored_keycodes_count < MAX_MONITORED_KEYCODES) {
                    ...
                    if (keycode >= 0 && keycode <= 0xFFFF) {  // Allow full 16-bit range
                        config->monitored_keycodes[config->monitored_keycodes_count++] = (uint32_t)keycode;
                        debug("Parsed monitored keycode: 0x%04x (%d)\n", keycode, keycode);
                    } else {
                        debugf(stderr, "Invalid keycode: 0x%04x in monitored_keycodes. Must be between 0 and 0xFFFF.\n", keycode);
                    }
                    token = strtok(NULL, ",");
                }
            }
            ...
            *eq = '\0';
            ...
            if (strcasecmp(key, "target") == 0) {
                ...
                if (current->binding_count >= MAX_BINDINGS) continue;
                if (strlen(val) < 2) continue; // Require at least mode+led
        ...
        if (dev->vendor == vendor && dev->product == product) {
            ...
            for (size_t j = 0; j < dev->binding_count; j++) {
                ...
                if (binding->keycode == keycode) {
                    ...
                    snprintf(cmd_buffer, sizeof(cmd_buffer), "%s %c%s",
                            config->setleds_path, binding->mode, binding->led);
                    return cmd_buffer;
                }
            }
